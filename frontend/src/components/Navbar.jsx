...
const Navbar = () => {
	...
	return (
		<header className='fixed top-0 left-0 w-full bg-white bg-opacity-95 backdrop-blur-md shadow z-40 transition-all duration-300 border-b border-blue-100'>
			<div className='container mx-auto px-4 py-3'>
				<div className='flex flex-wrap justify-between items-center'>
					...
					<nav className='flex flex-wrap items-center gap-4'>
						<Link to={'/'} className='text-slate-700 hover:text-blue-600 transition duration-300 ease-in-out'>
							Home
						</Link>
						{user && (
							<Link
						...
						{user ? (
							<button
								className='bg-slate-100 hover:bg-slate-50 text-slate-900 py-2 px-4 rounded-md flex items-center transition duration-300 ease-in-out'
								onClick={logout}
							>
								<LogOut size={18} />
